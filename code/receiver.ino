...
//    lcd.print(num/16);
        ...
        while(i < 8){
          ...
          sofe2b += e2*spare_datachain[ (itdata-8+i)%16 ];
          Serial.print(datachain[(itdata-8+i)%16]);
          Serial.print(" ");
          i++;
        }
        Serial.print("] - ");
        if(checkspare(sofe2b)){
          rest = sofe2b;
        }else{
          rest= sofe2;
        }
    ...
    if (rest == PS2_ENTER) {
      ...
      n=0;
      lcd.setCursor(n,m);
    } else if (rest == PS2_TAB) {
      n+=3;
      lcd.print("   ");
    } else if (rest == PS2_ESC) {
      n=0;
      m=0;
      lcd.clear();
    } else if (rest == PS2_PAGEDOWN) {
      m=(m+1)%2;
      lcd.setCursor(n,m);
    } else if (rest == PS2_PAGEUP) {
      m=(m-1)%2;
      lcd.setCursor(n,m);
    } else if (rest == PS2_LEFTARROW) {
      n = n-1;
      lcd.setCursor(n,m);
    } else if (rest == PS2_RIGHTARROW) {
      n=n+1;
      lcd.setCursor(n,m);
    } else if (rest == PS2_UPARROW) {
      m=(m-1)%2;
      lcd.setCursor(n,m);
    } else if (rest == PS2_DOWNARROW) {
      m=(m+1)%2;
      lcd.setCursor(n,m);
    } else if (rest == PS2_DELETE) {
      n=n-1;
      ...
      if(n <0){
       ...
       lcd.setCursor(n,m);
          ...
          // we need it not to print random words
        }
...
void s_reinitialize(){
  int iin = 0;
  while( iin<16 ){
    spare_datachain[ iin++ ] = 0;
  }
}
bool checkspare( int sof ){
  return (sof<123&&sof>96)||(sof>31&&sof < 91 && sof!=64&&sof!=48)||sof == 127 || sof==25||sof==26||sof == 27 ||sof==8||sof==10||sof==11||sof==21|| sof == 13|| sof==9;
...
void shiftright( int datum ){
  for(int i = 0 ; i<16-1; i++){
    spare_datachain[ i+1 ] = datachain[ i ];
    }
    
  spare_datachain[ 0 ] = datum;
}
